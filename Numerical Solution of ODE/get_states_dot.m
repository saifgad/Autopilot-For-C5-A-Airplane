function states_dot = get_states_dot(t,states_vector)
...
velocity_vect = [u; v; w];
J_matrix = [1, sin(phi)*tan(theta), cos(phi)*tan(theta); 0, cos(phi), -sin(phi); 0, sin(phi)/cos(theta), cos(phi)/cos(theta)];
...
states_dot(1:3 , 1) = F_vect/m - cross(angular_rates_vect, velocity_vect);
states_dot(4:6 , 1) = inv(I_matrix)*(M_vect - cross(angular_rates_vect, (I_matrix*angular_rates_vect) ));
